...
export class FacturaController {
    ...
    try {
      ...
      if (
        ...
        return res.status(400).json({
          message:
            "Datos inválidos: clienteId y detalles son requeridos.",
        });
      }
  ...
  static getAllFacturas = async (_req: Request, res: Response) => {
    try {
      ...
      if (facturas.length === 0) {
        return res
          .status(404)
          .json({ message: "No hay facturas registradas" });
      }

      return res.status(200).json({ ok: true, facturas: facturas });
  ...
  static getFacturaById = async (_req: Request, res: Response) => {
    try {
      ...
      return res.status(200).json({ ok: true, factura: factura });
